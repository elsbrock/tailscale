...
// TokenType is an enumeration specifying the type of S4U token to create.
type TokenType uint

const (
	// TokenTypeQuery is an S4U token that may only be used for querying
	// information about a user.
	TokenTypeQuery TokenType = iota
	// TokenTypeLogon is an S4U token that may be used for creating a S4U Session.
	TokenTypeLogon
)

// CreateToken creates a new S4U access token for user u for the purposes
// specified by s4uType. srcName must contain the name of the service that
// is intended to use the token. srcName must be non-empty, ASCII-only, and no
// longer than 8 characters.
//
// This API is useful enough to be public, but it is also fairly low-level;
// instead of calling CreateToken with s4uType == TokenTypeLogon, consider
// creating a Session by calling [Login].
//
// When s4uType is TokenTypeLogon, the current OS thread's access token must have SeTcbPrivilege.
func CreateToken(srcName string, u *user.User, s4uType TokenType) (tok windows.Token, err error) {
	if u == nil {
...
// profile, and returns a Session that may be used for creating processes as
// that user. srcName must be non-empty, ASCII-only, and no longer than 8
// characters. Logs will be written to logf, if provided.
//
// The current OS thread's access token must have SeTcbPrivilege.
func Login(logf logger.Logf, srcName string, u *user.User) (sess *Session, err error) {
	token, err := CreateToken(srcName, u, TokenTypeLogon)
	if err != nil {
		return nil, err
	}
	defer func() {
	...
	userProfile, err := winutil.LoadUserProfile(token, u)
	if err != nil {
		return nil, err
	}
	...
	return &Session{logf: logf, token: token, userProfile: userProfile}, nil
}
...
func startProcessInternal(ss *Session, logf logger.Logf, cmdLineInfo winutil.CommandLineInfo, opts startProcessOpts) (psp *Process, err error) {
	...
	if useToken {
		...
		wd, err := opts.token.GetUserProfileDirectory()
		if err != nil {
			return nil, err
		}
...
// Close cleans up any resources owned by sp. If the process was created with
// a pseudoconsole then the caller must continue concurrently draining sp's
// stdout until either Close finishes executing, or EOF.
