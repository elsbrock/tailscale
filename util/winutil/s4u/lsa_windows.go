...
// checkDomainAccount strips out the domain or computer name (if any) from
// username and returns the result in sanitizedUserName. isDomainAccount is set
// to true if username contained a domain component that does not refer to the
// local computer.
...
func (ls *lsaSession) LogonAs(srcName string, u *user.User) (token windows.Token, err error) {
	if l := len(srcName); l == 0 || l > _TOKEN_SOURCE_LENGTH {
		return token, fmt.Errorf("%w, actual length is %d", ErrBadSrcName, l)
	}
	if err := checkASCII(srcName); err != nil {
		return token, fmt.Errorf("%w: %v", ErrBadSrcName, err)
	}
	...
	if isDomainUser && !winenv.IsDomainJoined() {
		return token, fmt.Errorf("%w: cannot logon as domain user without being joined to a domain", os.ErrInvalid)
	}
	...
	if isDomainUser {
		...
		if err != nil {
			return token, fmt.Errorf("samToUPN16: %w", err)
		}
	...
	if e := wingoes.ErrorFromNTStatus(ntStatus); e.Failed() {
		return token, fmt.Errorf("LsaLogonUser(%q): %w, SubStatus: %v", u.Username, e, subNTStatus)
	}
	if profileBuf != 0 {
