...
// LogHandler's handled requests are logged using opts.Logf, as are any errors.
// Any ReturnHandler's requests' errors from within are added to the log.
...
// be passed back up the stack to logHandler.
...
func (h logHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	msg.Bytes = lw.bytes
	msg.Seconds = float64(h.opts.Now().Sub(msg.Time)) / float64(time.Second)
	switch {
	case lw.hijacked:
